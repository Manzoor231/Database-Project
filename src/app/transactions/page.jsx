  ...
  export default function TransactionsPage() {
    ...
    return (
      <div className="flex flex-col lg:flex-row min-h-screen bg-gray-50">
        ...
        <div className={`fixed inset-0 bg-black bg-opacity-30 z-30 lg:hidden ${openSidebar ? "block" : "hidden"}`} onClick={()=>setOpenSidebar(false)}></div>
        <aside className={`fixed top-0 left-0 h-full w-64 bg-white shadow-lg z-40 lg:hidden transform transition-transform ${openSidebar ? "translate-x-0" : "-translate-x-full"}`}>
          <Sidebar />
        </aside>

        <main className="flex-1 p-4 sm:p-6 space-y-6 lg:ml-4">
          {/* Header */}
          ...
          <Card className="p-4 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {[
                ...
                <FilterSelect key={f.key} label={f.label} value={filters[f.key]} onChange={v=>setFilters({...filters,[f.key]:v})} options={f.options}/>
              ))}
              <div>
